...
export function GameCard({ id, name, description, icon, color, index, cta }: GameCardProps) {
  const Icon = getGameIcon(icon);
  ...
  return (
    ...
    >
      <Link href={`/games/${id}` as '/games/memory'} className="block h-full group">
        ...
        >
          <div className="relative h-full glass-strong rounded-2xl border border-border/50 p-6 overflow-hidden transition-shadow duration-300 group-hover:shadow-xl">
            {/* Hover glow */}
            <div
